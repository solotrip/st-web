...
const PreferencesContainer = ({ children, page: Page, isOnboarding }) => {
  ...
  const eventInterestsSelectedCount = useSelector(
    eventInterestsSelectedCountSelector)
  const eventInterestsHasSelected = useSelector(eventInterestsHasSelectedSelector)
  const activityInterestsHasSelected = useSelector(activityInterestsHasSelectedSelector)
  ...
  const getNextEnabled = () => {
    switch (index) {
    case '1':
      return eventInterestsSelectedCount > 0 ||
        (isOnboarding && eventInterestsHasSelected)
    case '2':
      return activityInterestsSelectedCount > 0 ||
  ...
  return (<Page onNext={onNext} nextEnabled={nextEnabled} isGuest={isGuest}>
      {children}
    </Page>
  )
}
