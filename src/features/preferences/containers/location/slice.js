...
export const fetchBrowserGeolocation = createAsyncThunk(
  'profile/location/fetchBrowserGeolocation', async () => {
    return getGeolocation()
  }
)

export const updateLocation = createAsyncThunk(
  'profile/location/update', async (_, { getState, dispatch }) => {
    const {
      modified,
      coordinates
...
const locationSlice = createSlice({
  ...
  extraReducers: {
    ...
    [fetchProfile.fulfilled]: (state, action) => {
      ...
      if (!state.modified
        &&
        _.has(action, 'payload.location.coordinates[0]')
      ) {
        state.coordinates = {
...
export const locationSelector = state => state.preferences.location
